...
function gerarCamposMatriz() {
    ...
    document.getElementById("matrix-container").innerHTML += `<button onclick="calcularMatriz()">Calcular</button>`;
}
...
function calcularTransposta(matriz) {
    const transposta = matriz[0].map((_, colIndex) => matriz.map(row => row[colIndex]));
